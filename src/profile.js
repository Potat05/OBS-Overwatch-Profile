...
function setDOMProfile() {
    ...
    if(tank) document.getElementById("sr-tank").innerHTML = tank.level;
    const damage = profile.ratings.find(r => r.role == "damage");
    if(damage) document.getElementById("sr-damage").innerHTML = damage.level;
    const support = profile.ratings.find(r => r.role == "support");
    if(support) document.getElementById("sr-support").innerHTML = support.level;
    const flex = profile.ratings.find(r => r.role == "flex");
    if(flex) document.getElementById("sr-flex").innerHTML = flex.level;
}
